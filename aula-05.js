(() => {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .formalization-compare,.python-formalization-grid,.heuristic-example-grid,.heuristic-decision-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
      ...
      .formalization-card,.formalization-mini-example,.python-formalization-card,.heuristic-explainer-card,.heuristic-python-card,.heuristic-area-card,.heuristic-frontier-card,.algorithm-choice-card,.concept-anchor-card,.romania-summary-card,.romania-reading-grid article{border:1px solid var(--line);border-radius:20px;background:var(--paper);box-shadow:var(--shadow)}
      .formalization-card,.formalization-mini-example,.python-formalization-card,.heuristic-explainer-card,.heuristic-python-card,.heuristic-area-card,.heuristic-frontier-card,.algorithm-choice-card,.concept-anchor-card,.romania-summary-card{padding:1.2rem}
      .formalization-card h3,.formalization-mini-example h3,.python-formalization-card h3,.heuristic-explainer-card h3,.heuristic-python-card h3,.heuristic-area-card h3,.heuristic-frontier-card h3,.algorithm-choice-card h3,.concept-anchor-card h3{margin:0 0 .7rem}
      ...
      .formalization-mini-example,.python-formalization-grid,.heuristic-explainer-card,.heuristic-frontier-card,.heuristic-decision-grid,.heuristic-python-card,.heuristic-areas-grid{margin-top:1.1rem}
      .formalization-mini-example p,.concept-anchor-card p,.romania-summary-card>p,.heuristic-explainer-card p,.heuristic-python-card p,.heuristic-area-card p,.heuristic-frontier-card p,.algorithm-choice-card p{margin:0}
      ...
      .heuristic-explainer-card .formula-line,.heuristic-frontier-card .formula-line{margin-top:.9rem;padding:.85rem;border:1px solid var(--line);border-radius:14px;background:var(--soft);color:var(--ink)}
      .heuristic-explainer-card .formula-line strong,.heuristic-frontier-card .formula-line strong{color:var(--blue)}
      .heuristic-decision-grid{display:grid;gap:14px;margin-top:1.1rem}
      .algorithm-choice-card{display:grid;gap:.75rem}
      .algorithm-choice-card .choice-rule{display:inline-flex;width:max-content;max-width:100%;padding:.35rem .65rem;border-radius:999px;background:var(--soft);border:1px solid var(--line);font-weight:850}
      ...
      .guided-simulator{grid-template-columns:1fr}.guided-graph-card,.guided-control-card{width:100%}
      .romania-summary{min-height:0!important;padding:0;border:0;background:transparent;overflow:visible}.romania-summary-card>p{margin-bottom:1rem}
      ...
      body.theme-dark .formalization-card,body.theme-dark .formalization-mini-example,body.theme-dark .python-formalization-card,body.theme-dark .heuristic-explainer-card,body.theme-dark .heuristic-python-card,body.theme-dark .heuristic-area-card,body.theme-dark .heuristic-frontier-card,body.theme-dark .algorithm-choice-card,body.theme-dark .concept-anchor-card,body.theme-dark .concept-anchor-grid article,body.theme-dark .romania-summary-card,body.theme-dark .romania-summary-grid article,body.theme-dark .romania-reading-grid article{background:var(--paper);border-color:var(--line)}
      body.theme-dark .formalization-list li,body.theme-dark .formalization-table th,body.theme-dark .heuristic-frontier-table th,body.theme-dark .heuristic-explainer-card .formula-line,body.theme-dark .heuristic-frontier-card .formula-line,body.theme-dark .heuristic-card-note,body.theme-dark .algorithm-choice-card .choice-rule,body.theme-dark .algorithm-choice-card li{background:var(--soft)}
      @media(max-width:980px){#visao .overview-grid,.formalization-compare,.python-formalization-grid,.heuristic-example-grid,.heuristic-decision-grid,.heuristic-areas-grid,.concept-anchor-grid,.romania-summary-grid,.romania-reading-grid,.guided-status-grid{grid-template-columns:1fr}}
    `;
  ...
  const insertFormalizationSection=()=>{const percurso=document.querySelector('#percurso');if(!percurso||document.querySelector('#formalizacao-informada'))return;percurso.insertAdjacentHTML('afterend',`
...
# A*: prioridade = g(n) + h(n)</code></pre></article></div>
  ...
  const updatePromptBox=()=>{const title=document.querySelector('#heuristica .prompt-box strong');if(title)title.textContent='Para pensar durante a aula';};
  ...
  const enhanceHeuristicSection=()=>{const container=document.querySelector('#heuristica .container');const callout=container?.querySelector('.heuristic-callout');if(!container||!callout||container.querySelector('.heuristic-frontier-card'))return;callout.insertAdjacentHTML('afterend',`
...
print("A* escolhe:", escolha_astar["cidade"])</code></pre></article>
  ...
  const configureRomaniaReference=()=>{const section=root.closest('section');const sectionTitle=section?.querySelector('.section-heading h2');const sectionIntro=section?.querySelector('.section-heading p:last-child');const graphTitle=root.querySelector('.graph-heading h3');const graphIntro=root.querySelector('.graph-heading p');const graphLegend=root.querySelector('.graph-legend');const graph=root.querySelector('.guided-graph');if(sectionTitle)sectionTitle.textContent='Greedy e A* no mesmo problema de rotas';if(sectionIntro)sectionIntro.textContent='Retomamos o problema contextualizado na formulação inicial. Agora o foco é observar como a fronteira muda quando usamos apenas h(n) ou a soma g(n)+h(n).';if(graphTitle)graphTitle.textContent='Retomando o cenário';if(graphIntro)graphIntro.innerHTML='Objetivo: sair de <strong>Arad</strong> e chegar a <strong>Bucharest</strong> com menor custo.';if(graphLegend)graphLegend.innerHTML='<span>Busca gulosa: prioriza menor h(n)</span><span>A*: prioriza menor f(n)=g(n)+h(n)</span>';if(graph){graph.classList.add('romania-summary');graph.setAttribute('aria-label','Resumo do problema da Romênia para comparação entre busca gulosa e A estrela');graph.innerHTML=`<div class="romania-summary-card"><p>O grafo continua sendo o mesmo. O que muda é a informação usada para organizar a fronteira e decidir qual alternativa explorar primeiro.</p><div class="romania-summary-grid"><article><strong>Estado inicial e objetivo</strong><span>Partimos de Arad e queremos chegar a Bucharest.</span></article><article><strong>Ponto crítico da comparação</strong><span>Em Sibiu, a escolha entre Fagaras e Rimnicu Vilcea mostra bem a diferença entre usar só h(n) ou combinar g(n)+h(n).</span></article><article><strong>Resultado esperado</strong><span>Greedy tende a seguir Arad → Sibiu → Fagaras. A* acaba preferindo a rota por Rimnicu Vilcea e Pitesti.</span></article></div></div>`;}if(!section?.querySelector('.romania-reading-grid')){const graphCard=root.querySelector('.guided-graph-card');graphCard?.insertAdjacentHTML('beforeend',`<div class="romania-reading-grid"><article><small>01</small><h3>O que o problema já tinha?</h3><p>Estados, conexões entre cidades e custo real das estradas.</p></article><article><small>02</small><h3>O que foi acrescentado?</h3><p>Uma heurística h(n), que funciona como uma estimativa direta até Bucharest.</p></article><article><small>03</small><h3>O que isso permite comparar?</h3><p>Agora podemos observar a diferença entre ordenar a fronteira por h(n) ou por g(n)+h(n).</p></article></div>`);}const costExample=document.querySelector('.cost-example');if(costExample){const blocks=costExample.querySelectorAll('div');if(blocks[0])blocks[0].innerHTML='<small>Distância já percorrida até Pitesti</small><strong>g(n) = 317</strong>';if(blocks[1])blocks[1].innerHTML='<small>Estimativa direta de Pitesti até Bucharest</small><strong>h(n) = 100</strong>';if(blocks[2])blocks[2].innerHTML='<small>Estimativa total por Pitesti</small><strong>f(n) = 417</strong>';}const questions=section?.querySelectorAll('.experiment-questions article');if(questions?.[0])questions[0].querySelector('p').textContent='Registre o caminho encontrado e some os custos das estradas.';if(questions?.[1])questions[1].querySelector('p').textContent='Observe o momento em que a rota aparentemente direta por Fagaras perde para a alternativa por Rimnicu Vilcea e Pitesti.';if(questions?.[2])questions[2].querySelector('p').textContent='Fagaras parece mais perto de Bucharest pela heurística, mas o custo real Fagaras → Bucharest é alto.';const tableBody=document.querySelector('.heuristic-check-table tbody');if(tableBody)tableBody.innerHTML=`<tr><td>Arad</td><td>366</td><td>418</td><td><strong>Sim</strong></td></tr><tr><td>Sibiu</td><td>253</td><td>278</td><td><strong>Sim</strong></td></tr><tr><td>Fagaras</td><td>176</td><td>211</td><td><strong>Sim</strong></td></tr><tr><td>Rimnicu Vilcea</td><td>193</td><td>198</td><td><strong>Sim</strong></td></tr><tr><td>Pitesti</td><td>100</td><td>101</td><td><strong>Sim</strong></td></tr><tr><td>Bucharest</td><td>0</td><td>0</td><td><strong>Sim</strong></td></tr>`;};

  injectStyles();organizeTopicList();compactCompetencyCards();insertFormalizationSection();updateBridgeText();updatePromptBox();updateHeuristicIntroCards();updateHeuristicCallout();enhanceHeuristicSection();insertConceptAnchorCard();configureRomaniaReference();

  const modeButtons=[...root.querySelectorAll('[data-mode]')];const stepButton=root.querySelector('[data-step]');const resetButton=root.querySelector('[data-reset]');const currentEl=root.querySelector('[data-current]');const frontierEl=root.querySelector('[data-frontier]');const costEl=root.querySelector('[data-cost]');const pathEl=root.querySelector('[data-path]');const explanationEl=root.querySelector('[data-explanation]');const outcomeEl=root.querySelector('[data-outcome]');
  const frames={greedy:[{current:'—',frontier:'Arad · h=366',cost:'0',path:'Arad',explanation:'Começamos em Arad. Na busca gulosa, a fronteira será ordenada somente pela menor estimativa h(n) até Bucharest.'},{current:'Arad',frontier:'Sibiu · h=253  |  Timisoara · h=329  |  Zerind · h=374',cost:'0',path:'Arad',explanation:'Ao expandir Arad, aparecem três alternativas. Sibiu parece mais perto de Bucharest, então é escolhido primeiro.'},{current:'Sibiu',frontier:'Fagaras · h=176  |  Rimnicu Vilcea · h=193  |  Timisoara · h=329  |  Zerind · h=374  |  Oradea · h=380',cost:'140',path:'Arad → Sibiu',explanation:'Sibiu gera Fagaras e Rimnicu Vilcea. Como Fagaras tem o menor h(n), a busca gulosa segue por ele.'},{current:'Fagaras',frontier:'Bucharest · h=0  |  Rimnicu Vilcea · h=193  |  Timisoara · h=329  |  Zerind · h=374  |  Oradea · h=380',cost:'239',path:'Arad → Sibiu → Fagaras',explanation:'Fagaras parece excelente pela heurística. Ao expandi-lo, Bucharest entra na fronteira com h=0.'},{current:'Bucharest',frontier:'Rimnicu Vilcea · h=193  |  Timisoara · h=329  |  Zerind · h=374  |  Oradea · h=380',cost:'450',path:'Arad → Sibiu → Fagaras → Bucharest',explanation:'Bucharest é retirado da fronteira e satisfaz o objetivo. A busca termina sem investigar a alternativa por Rimnicu Vilcea e Pitesti.',outcome:'Busca gulosa: solução com custo 450. Foi direta, mas não encontrou a rota de menor custo.'}],astar:[{current:'—',frontier:'Arad · f=366',cost:'0',path:'Arad',explanation:'Começamos em Arad. No A*, cada prioridade é calculada por f(n)=g(n)+h(n).'},{current:'Arad',frontier:'Sibiu · f=393  |  Timisoara · f=447  |  Zerind · f=449',cost:'0',path:'Arad',explanation:'Depois de expandir Arad: Sibiu tem g=140 e h=253, então f=393. É a menor prioridade da fronteira.'},{current:'Sibiu',frontier:'Rimnicu Vilcea · f=413  |  Fagaras · f=415  |  Timisoara · f=447  |  Zerind · f=449  |  Oradea · f=671',cost:'140',path:'Arad → Sibiu',explanation:'Sibiu gera Rimnicu Vilcea e Fagaras. A rota por Rimnicu Vilcea tem f=220+193=413, ligeiramente melhor que Fagaras.'},{current:'Rimnicu Vilcea',frontier:'Fagaras · f=415  |  Pitesti · f=417  |  Timisoara · f=447  |  Zerind · f=449  |  Craiova · f=526  |  Oradea · f=671',cost:'220',path:'Arad → Sibiu → Rimnicu Vilcea',explanation:'Rimnicu Vilcea aproxima o algoritmo de Pitesti, mas Fagaras ainda tem f um pouco menor. O A* mantém as alternativas concorrendo.'},{current:'Fagaras',frontier:'Pitesti · f=417  |  Timisoara · f=447  |  Zerind · f=449  |  Bucharest · f=450  |  Craiova · f=526  |  Oradea · f=671',cost:'239',path:'Arad → Sibiu → Fagaras',explanation:'Fagaras gera Bucharest com custo total 450. O objetivo apareceu, mas não é escolhido ainda porque Pitesti tem f=417.'},{current:'Pitesti',frontier:'Bucharest · f=418  |  Timisoara · f=447  |  Zerind · f=449  |  Craiova · f=526  |  Oradea · f=671',cost:'317',path:'Arad → Sibiu → Rimnicu Vilcea → Pitesti',explanation:'Pitesti gera uma rota melhor para Bucharest: g=418 e h=0. Agora Bucharest passa a ter a menor prioridade.'},{current:'Bucharest',frontier:'Timisoara · f=447  |  Zerind · f=449  |  Craiova · f=526  |  Oradea · f=671',cost:'418',path:'Arad → Sibiu → Rimnicu Vilcea → Pitesti → Bucharest',explanation:'Bucharest é o menor estado da fronteira e satisfaz o objetivo. O caminho encontrado custa 418.',outcome:'A*: solução com custo 418. Ao combinar g(n) e h(n), evitou aceitar a rota mais cara por Fagaras.'}]};
  let mode='greedy';let frameIndex=0;const render=()=>{const frame=frames[mode][frameIndex];if(!frame)return;currentEl.textContent=frame.current;frontierEl.textContent=frame.frontier;costEl.textContent=frame.cost;pathEl.textContent=frame.path;explanationEl.textContent=frame.explanation;outcomeEl.textContent=frame.outcome||'';const atEnd=frameIndex===frames[mode].length-1;stepButton.disabled=atEnd;stepButton.textContent=atEnd?'Execução concluída':'Executar um passo';};const reset=()=>{frameIndex=0;render();};modeButtons.forEach((button)=>button.addEventListener('click',()=>{mode=button.dataset.mode;modeButtons.forEach((item)=>item.classList.toggle('active',item===button));reset();}));stepButton.addEventListener('click',()=>{if(frameIndex<frames[mode].length-1){frameIndex+=1;render();}});resetButton.addEventListener('click',reset);render();
})();