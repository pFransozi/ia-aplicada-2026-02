(() => {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .formalization-compare, .python-formalization-grid, .heuristic-example-grid, .heuristic-decision-grid, .heuristic-areas-grid, .romania-summary-grid, .romania-reading-grid, .guided-status-grid, .algorithm-didactic-grid, .comparison-panels, .map-status-strip, .runner-stats-grid { display:grid; gap:14px; }
      .formalization-compare, .python-formalization-grid, .heuristic-example-grid, .heuristic-decision-grid, .algorithm-didactic-grid, .comparison-panels, .map-status-strip, .runner-stats-grid { grid-template-columns:repeat(2,minmax(0,1fr)); }
      ...
      .formalization-mini-example, .python-formalization-grid, .heuristic-explainer-card, .heuristic-frontier-card, .heuristic-decision-grid, .heuristic-python-card, .heuristic-areas-grid, .algorithm-didactic-grid { margin-top:1.1rem; }
      .formalization-mini-example p, .concept-anchor-card p, .romania-summary-card > p, .heuristic-explainer-card p, .heuristic-frontier-card p, .heuristic-python-card p, .heuristic-area-card p, .algorithm-choice-card p, .algorithm-pseudo-card p, .algorithm-code-card p, .runner-explanation-card p { margin:0; }
      ...
      .pseudo-list { display:grid; gap:.55rem; margin:1rem 0 0; padding-left:1.25rem; }
      .pseudo-list li { line-height:1.5; }
      ...
      @media(max-width:980px){#visao .overview-grid,.formalization-compare,.python-formalization-grid,.heuristic-example-grid,.heuristic-decision-grid,.heuristic-areas-grid,.algorithm-didactic-grid,.comparison-panels,.map-status-strip,.concept-anchor-grid,.romania-summary-grid,.romania-reading-grid,.guided-status-grid{grid-template-columns:1fr}}
  ...
  const organizeTopicList = () => { const list = document.querySelector('#visao .topic-list'); if (!list || list.dataset.ordered === 'true') return; const topics = ['Busca informada','Função heurística h(n)','Busca gulosa','Custo acumulado g(n)','Função f(n) = g(n) + h(n)','Algoritmo A*','Admissibilidade','Comparação de estratégias']; list.innerHTML = topics.map((topic) => `<div>${topic}</div>`).join(''); list.dataset.ordered = 'true'; };
  const compactCompetencyCards = () => { const overview = document.querySelector('#visao .overview-grid'); if (!overview || overview.querySelector('.competency-stack')) return; const cards = [...overview.querySelectorAll(':scope > article.competency')]; if (!cards.length) return; const stack = document.createElement('div'); stack.className = 'competency-stack'; cards[0].before(stack); cards.forEach((card) => stack.appendChild(card)); };
...
print("A* escolhe:", escolha_astar["cidade"])</code></pre></article><div class="heuristic-explainer-card"><h3>Por que Greedy e A* escolhem caminhos diferentes?</h3><p>A busca gulosa segue a alternativa que parece mais próxima do objetivo. O A* tenta equilibrar essa proximidade estimada com o custo real já acumulado.</p><div class="formula-line"><strong>Ideia central:</strong> h(n) ajuda a enxergar uma direção provável. g(n) lembra o custo que já foi pago. A* usa as duas informações.</div></div><div class="heuristic-areas-grid"><article class="heuristic-area-card"><small>Exemplo 01</small><h3>Rotas e logística</h3><p>Em rotas, h(n) pode estimar a distância direta até o destino. Essa pista ignora curvas, trânsito e pedágios, mas ajuda a evitar explorar cidades que apontam claramente para longe do objetivo.</p><span>Estado: cidade atual. Heurística: distância direta estimada até a cidade destino.</span></article><article class="heuristic-area-card"><small>Exemplo 02</small><h3>Jogos e labirintos</h3><p>Em um grid ou labirinto, h(n) pode ser a distância Manhattan: quantas linhas e colunas faltam até a saída, ignorando paredes temporariamente.</p><span>Estado: posição atual. Heurística: |linha atual - linha final| + |coluna atual - coluna final|.</span></article><article class="heuristic-area-card"><small>Exemplo 03</small><h3>Planejamento de tarefas</h3><p>Em planejamento, h(n) pode estimar quantas tarefas críticas ainda faltam para concluir o objetivo. A busca prioriza planos que parecem reduzir pendências mais rapidamente.</p><span>Estado: tarefas concluídas. Heurística: estimativa do esforço restante.</span></article></div>`); };
  const insertConceptAnchorCard = () => { const container = document.querySelector('#heuristica .container'); const target = container?.querySelector('.heuristic-areas-grid') || container?.querySelector('.heuristic-callout'); if (!container || !target || container.querySelector('.concept-anchor-card')) return; target.insertAdjacentHTML('afterend', `<div class="concept-anchor-card"><h3>Como avaliar se uma heurística ajuda?</h3><p>Uma heurística precisa ser relacionada ao domínio do problema, barata de calcular e útil para ordenar a fronteira. Em alguns algoritmos, como A*, também precisamos discutir se ela preserva garantias sobre a qualidade da solução.</p><div class="concept-anchor-grid"><article><strong>Domínio</strong><span>A estimativa precisa ter relação com o objetivo.</span></article><article><strong>Custo de cálculo</strong><span>Calcular h(n) não pode ser tão caro quanto resolver o problema.</span></article><article><strong>Qualidade da pista</strong><span>Quanto melhor a estimativa, menor tende a ser a exploração desnecessária.</span></article><article><strong>Garantia</strong><span>Para discutir otimalidade no A*, a admissibilidade passa a importar.</span></article></div></div>`); };

  const configureRomaniaReference = () => { if (!root) return; const section = root.closest('section'); const sectionTitle = section?.querySelector('.section-heading h2'); const sectionIntro = section?.querySelector('.section-heading p:last-child'); if (sectionTitle) sectionTitle.textContent = 'Greedy e A* no mesmo problema de rotas'; if (sectionIntro) sectionIntro.textContent = 'Retomamos o problema contextualizado na formulação inicial. Agora o foco é observar como a fronteira muda quando usamos apenas h(n) ou a soma g(n)+h(n).'; const costExample = document.querySelector('.cost-example'); if (costExample) { const blocks = costExample.querySelectorAll('div'); if (blocks[0]) blocks[0].innerHTML = '<small>Distância já percorrida até Pitesti</small><strong>g(n) = 317</strong>'; if (blocks[1]) blocks[1].innerHTML = '<small>Estimativa direta de Pitesti até Bucharest</small><strong>h(n) = 100</strong>'; if (blocks[2]) blocks[2].innerHTML = '<small>Estimativa total por Pitesti</small><strong>f(n) = 417</strong>'; } const questions = section?.querySelectorAll('.experiment-questions article'); if (questions?.[0]) questions[0].querySelector('p').textContent = 'Registre o caminho encontrado e some os custos das estradas.'; if (questions?.[1]) questions[1].querySelector('p').textContent = 'Observe o momento em que a rota aparentemente direta por Fagaras perde para a alternativa por Rimnicu Vilcea e Pitesti.'; if (questions?.[2]) questions[2].querySelector('p').textContent = 'Fagaras parece mais perto de Bucharest pela heurística, mas o custo real Fagaras → Bucharest é alto.'; const tableBody = document.querySelector('.heuristic-check-table tbody'); if (tableBody) tableBody.innerHTML = `<tr><td>Arad</td><td>366</td><td>418</td><td><strong>Sim</strong></td></tr><tr><td>Sibiu</td><td>253</td><td>278</td><td><strong>Sim</strong></td></tr><tr><td>Fagaras</td><td>176</td><td>211</td><td><strong>Sim</strong></td></tr><tr><td>Rimnicu Vilcea</td><td>193</td><td>198</td><td><strong>Sim</strong></td></tr><tr><td>Pitesti</td><td>100</td><td>101</td><td><strong>Sim</strong></td></tr><tr><td>Bucharest</td><td>0</td><td>0</td><td><strong>Sim</strong></td></tr>`; };

  const insertAlgorithmDidacticPanels = () => { const greedySection = document.querySelector('#gulosa'); const astarSection = document.querySelector('#astar'); if (greedySection && !greedySection.querySelector('.algorithm-didactic-grid')) { greedySection.querySelector('.algorithm-layout')?.classList.add('legacy-algorithm-layout-hidden'); const note = greedySection.querySelector('.note-banner'); const html = `<div class="algorithm-didactic-grid"><article class="algorithm-pseudo-card"><span class="didactic-label">Pseudocódigo</span><h3>Como a gulosa decide?</h3><p>Ela escolhe, na fronteira, o estado que parece mais próximo do objetivo segundo <strong>h(n)</strong>.</p><ol class="pseudo-list"><li>colocar o estado inicial na fronteira</li><li>enquanto a fronteira não estiver vazia</li><li>escolher o estado com menor <strong>h(n)</strong></li><li>se ele for o objetivo, devolver a solução</li><li>expandir o estado atual</li><li>inserir os sucessores na fronteira</li><li>se a fronteira acabar, devolver fracasso</li></ol><div class="algorithm-reading-note"><strong>Leitura didática:</strong> a gulosa pergunta: <em>qual estado parece mais perto do objetivo agora?</em></div></article><article class="algorithm-code-card"><span class="didactic-label">Python</span><h3>Exemplo mínimo em Python</h3><p>A prioridade da fila é o menor valor de <strong>h(n)</strong>.</p><pre><code>from heapq import heappush, heappop
...
def busca_gulosa(inicio, objetivo, sucessores, h):
    ...
    return None</code></pre><div class="algorithm-reading-note"><strong>Observe:</strong> o problema pode ter custo nas estradas, mas a gulosa escolhe usando apenas <strong>h(vizinho)</strong>.</div></article></div>`; if (note) note.insertAdjacentHTML('beforebegin', html); }
    if (astarSection && !astarSection.querySelector('.algorithm-didactic-grid')) { astarSection.querySelector('.algorithm-layout')?.classList.add('legacy-algorithm-layout-hidden'); astarSection.querySelector('.container')?.insertAdjacentHTML('beforeend', `<div class="algorithm-didactic-grid"><article class="algorithm-pseudo-card"><span class="didactic-label">Pseudocódigo</span><h3>Como o A* decide?</h3><p>O A* combina o custo já acumulado com a estimativa restante.</p><div class="formula-emphasis">f(n) = g(n) + h(n)</div><ol class="pseudo-list"><li>colocar o estado inicial na fronteira</li><li>registrar <strong>g(inicial) = 0</strong></li><li>enquanto a fronteira não estiver vazia</li><li>escolher o estado com menor <strong>f(n)</strong></li><li>se ele for o objetivo, devolver a solução</li><li>expandir o estado atual</li><li>para cada sucessor, calcular o novo custo acumulado</li><li>atualizar a fronteira se esse caminho for melhor</li><li>se a fronteira acabar, devolver fracasso</li></ol><div class="algorithm-reading-note"><strong>Leitura didática:</strong> o A* pergunta: <em>qual estado parece mais promissor considerando o que já custou e o que ainda parece faltar?</em></div></article><article class="algorithm-code-card"><span class="didactic-label">Python</span><h3>Exemplo mínimo em Python</h3><p>A prioridade da fila passa a ser <strong>g(n) + h(n)</strong>.</p><pre><code>from heapq import heappush, heappop
...
def a_estrela(inicio, objetivo, sucessores, h):
  ...
  const comparisonFrames = { greedy: [{step:0,current:'Arad',cost:'0',path:['Arad'],frontier:['Arad · h=366'],frontierCities:[],explanation:'Começamos em Arad. A gulosa prioriza sempre o menor valor de h(n).',outcome:''},{step:1,current:'Arad',cost:'0',path:['Arad'],frontier:['Sibiu · h=253','Timisoara · h=329','Zerind · h=374'],frontierCities:['Sibiu','Timisoara','Zerind'],explanation:'Ao expandir Arad, Sibiu parece a melhor escolha porque tem a menor estimativa até Bucharest.',outcome:''},{step:2,current:'Sibiu',cost:'140',path:['Arad','Sibiu'],frontier:['Fagaras · h=176','Rimnicu Vilcea · h=193','Timisoara · h=329','Zerind · h=374','Oradea · h=380'],frontierCities:['Fagaras','Rimnicu Vilcea','Timisoara','Zerind','Oradea'],explanation:'Em Sibiu, a gulosa olha apenas para h(n). Fagaras parece melhor do que Rimnicu Vilcea.',outcome:''},{step:3,current:'Fagaras',cost:'239',path:['Arad','Sibiu','Fagaras'],frontier:['Bucharest · h=0','Rimnicu Vilcea · h=193','Timisoara · h=329','Zerind · h=374','Oradea · h=380'],frontierCities:['Bucharest','Rimnicu Vilcea','Timisoara','Zerind','Oradea'],explanation:'Ao expandir Fagaras, Bucharest entra com h=0. Para a gulosa, isso torna a escolha imediata.',outcome:''},{step:4,current:'Bucharest',cost:'450',path:['Arad','Sibiu','Fagaras','Bucharest'],frontier:['Rimnicu Vilcea · h=193','Timisoara · h=329','Zerind · h=374','Oradea · h=380'],frontierCities:['Rimnicu Vilcea','Timisoara','Zerind','Oradea'],explanation:'A gulosa chega ao objetivo rapidamente, mas sem garantir o melhor custo.',outcome:'Resultado: caminho encontrado com custo 450.'}], astar: [{step:0,current:'Arad',cost:'0',path:['Arad'],frontier:['Arad · f=366'],frontierCities:[],explanation:'Começamos em Arad. No A*, a prioridade é f(n) = g(n) + h(n).',outcome:''},{step:1,current:'Arad',cost:'0',path:['Arad'],frontier:['Sibiu · f=393','Timisoara · f=447','Zerind · f=449'],frontierCities:['Sibiu','Timisoara','Zerind'],explanation:'Depois de expandir Arad, Sibiu fica com a melhor prioridade total.',outcome:''},{step:2,current:'Sibiu',cost:'140',path:['Arad','Sibiu'],frontier:['Rimnicu Vilcea · f=413','Fagaras · f=415','Timisoara · f=447','Zerind · f=449','Oradea · f=671'],frontierCities:['Rimnicu Vilcea','Fagaras','Timisoara','Zerind','Oradea'],explanation:'Aqui aparece a diferença central: Rimnicu Vilcea vence Fagaras por pouco quando usamos g(n)+h(n).',outcome:''},{step:3,current:'Rimnicu Vilcea',cost:'220',path:['Arad','Sibiu','Rimnicu Vilcea'],frontier:['Fagaras · f=415','Pitesti · f=417','Timisoara · f=447','Zerind · f=449','Craiova · f=526','Oradea · f=671'],frontierCities:['Fagaras','Pitesti','Timisoara','Zerind','Craiova','Oradea'],explanation:'O A* mantém alternativas vivas. Fagaras ainda está na fronteira, mas Pitesti já se aproxima.',outcome:''},{step:4,current:'Fagaras',cost:'239',path:['Arad','Sibiu','Fagaras'],frontier:['Pitesti · f=417','Timisoara · f=447','Zerind · f=449','Bucharest · f=450','Craiova · f=526','Oradea · f=671'],frontierCities:['Pitesti','Timisoara','Zerind','Bucharest','Craiova','Oradea'],explanation:'Fagaras gera uma rota até Bucharest, mas ainda não é a melhor prioridade disponível.',outcome:''},{step:5,current:'Pitesti',cost:'317',path:['Arad','Sibiu','Rimnicu Vilcea','Pitesti'],frontier:['Bucharest · f=418','Timisoara · f=447','Zerind · f=449','Craiova · f=526','Oradea · f=671'],frontierCities:['Bucharest','Timisoara','Zerind','Craiova','Oradea'],explanation:'Pitesti melhora a rota até Bucharest. Agora o objetivo entra com custo total menor.',outcome:''},{step:6,current:'Bucharest',cost:'418',path:['Arad','Sibiu','Rimnicu Vilcea','Pitesti','Bucharest'],frontier:['Timisoara · f=447','Zerind · f=449','Craiova · f=526','Oradea · f=671'],frontierCities:['Timisoara','Zerind','Craiova','Oradea'],explanation:'O A* chega ao objetivo com menor custo total do que a busca gulosa.',outcome:'Resultado: caminho encontrado com custo 418.'}] };
